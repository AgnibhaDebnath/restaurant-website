...
const Menu = () => {
    ...
    return (
        <section id='menu' className='my-3 bg-[#f9fafb] scroll-mt-20'>
            ...
            <div className='w-full grid grid-cols-1 min-[988px]:grid-cols-2 min-[1450px]:grid-cols-3 place-items-center justify-items-center gap-10 px-10'>
                {filteredMenu.map((item) => (
                    <>
                        {< div key={item.id} className='w-80 min-[385px]:w-90 min-[432px]:w-100 min-[500px]:w-115 rounded-3xl px-7 min-[500px]:px-10 pt-8 pb-3 bg-white shadow-xl overflow-hidden hover:shadow-2xl' >
                            ...
                            <div className='py-1 min-[432px]:py-3'>
                                <h1 className='font-extrabold text-2xl font-[inter]'>{item.name}</h1>
                                <h1 className='font-medium text-[#f59e0b] font-[inter] text-lg'>{item.price}</h1>
                            </div>
                        </div>}
                    </>
                ))}
